...
- Remote GitHub Actions CI #50-#54 failed before tests at `npm ci`; npm 10 reported missing optional peers `@emnapi/core@2.0.0-alpha.5` and `@emnapi/runtime@2.0.0-alpha.5`. Local commit `18fd2f8` synchronizes those lock entries, and `npx npm@10.9.2 ci --dry-run --ignore-scripts` passes. CI #55 passed `npm ci` and dependency setup, then failed in `npm run test:e2e` at `agent-prompt-policy.test.js` because the test depended on the shared default CDP port. CI #56 used explicit `http://127.0.0.1:0` but still exposed an asynchronous shutdown race: `mcp/server.mjs` exited on stdin close before the `tools/list` response was written. Local commit `3862047` now drains pending requests before shutdown; Node 22/25 focused tests, 100-repeat RPC, and the full local build pass. Follow-up local commit `0cb3cc0` aligns Demo isolated-runtime fixtures, synthetic pixel policy admission and cross-browser resize key dispatch; CI-mode full local E2E completed **475 passed, 1 flaky Service Worker cache-warm-up case, 34 skipped, 0 final failures**. A remote rerun of these amendments is not claimed.
- `npm run check` — pass; PI-04 source/build `node --check` — pass; `git diff --check` — pass.
- Offline quality follow-up (2026-09-11): `npm run doctor` passed **5/5**; the mock provider/privacy focused suite passed **9 files / 39 tests**; and `npm ci --dry-run --ignore-scripts` exited 0. `npm sbom --sbom-format=cyclonedx --sbom-type=application` reported **196 components**. `npm pack --dry-run` reported **550 files** and warned that `.npmignore` is absent; the local dependency tree reports one extraneous `@emnapi/wasi-threads@2.1.0`, with no tracked manifest or lockfile change. Secretlint was not run because the repository has no `.secretlintrc` configuration; this is an unrun security check, not a pass. These findings do not add S17/PI-04 gate credit.
...
- `.github/workflows/ci.yml` declares all three static pilot validators; a local structural check found exactly three entries. CI #55 did not reach those validators because the preceding unit/build stage failed; the amendment has not been pushed or remotely rerun.
