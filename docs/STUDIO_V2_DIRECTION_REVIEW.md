...
|---|---|---|
| R1 / High | app.js retains activeSampleKey on import. Its exact onDataPolicyToggle function, invoked with an imported document and that retained key, returned Synthetic with allowDurable=true. Initial Unknown import does not protect a later loosening transition. | Host must use current-document provenance/declaration, not prior sample UI state. DP-C07/DP-L03; 13-08 is Fail. |
| R2 / High | AgentSessionManager.create starts index opening under Synthetic, awaits it, then issues put without checking the new policy. A fake IndexedDB sink observed a label write after switching to Real. | Bind work to document/session/policy generation and recheck before actual store admission and after awaits. DP-L02/04; 13-04 is reopened as Fail. |
| R3 / High | new AgentSessionManager() assigns Synthetic without an explicit policy. Gateway defaults cannot compensate for a different session-owner default. | Missing policy must be restrictive at the session owner. Preserve explicit synthetic positive controls. 13-08 remains Fail. |
...
- Session fallback: resumed tests now cover delayed index/runtime opening and transaction abort after request success; the expanded asynchronous runtime-write failure composition is still being reviewed. A synchronous factory catch is not sufficient evidence for asynchronous persistence.
...
- Recipient lifecycle: profile selection changes provider state without clearly establishing a fresh conversation context. Verify the final request after recipient replacement; do not infer safety from tool output or a UI label.
- Explicit save: current save code checks document/revision after picker completion, but does not show the required current-policy guard at each side-effect boundary. Preserve confirmed writes and distinguish stale results from failed writes.
...
- All-35 command coverage is useful supporting evidence, not all-35 acceptance-case closure. This review did not re-audit every return variant or run a new full suite.
...
Current P0 register: **5 Pass, 2 Fail, 28 Not run** (35 total).

- Retained bounded Pass records: 13-01/02/03/05/06, only for their documented tested sequences.
- 13-04: Fail, reopened by the delayed session-write probe; previous Provider-delivery passes are retained as narrower evidence.
- 13-08: Fail, confirmed provenance/default-policy defects. The earlier isolated Chromium storage-denial subcase passed but does not close the complete case.
- 13-07: complete acceptance remains Not run; a latest-run marker without a reviewed case trace is not sufficient closure evidence.
- Prior 86 files / 454 unit tests, 54/54 targeted browser tests, 190/222 combined browser tests and doctor 5/5 are pre-review snapshot evidence. They were not rerun for this documentation-only correction or all latest session edits.
...
- R5 / High / Consistent: mcp/server.mjs requires full-page browser screenshots without a Synthetic condition. This conflicts with DP-T02 for Unknown/Real and must not encourage a screenshot bypass. No actual screenshot disclosure was executed or established by this review.
- R6 / Medium / Clear: agent-designer-prompt.js allows raw source replacement when explicitly requested, whereas the gateway rejects that Agent surface unconditionally. Correct the prompt during the resumed implementation; preserve the semantic/human-editor boundary.
...
- Verify the exact composed/loaded instructions plus host decisions and final Provider requests after the code correction. A document update or a cooperative model response cannot establish enforcement. Existing P0 counts are unchanged; complete prompt/host agreement remains unverified.
...
1. Retain the implemented R1-R3 corrections and finish their complete side-effect/late-result acceptance. Do not replace actual store admission guards with factory-only checks or discard committed memory state.
2. Close the remaining recipient transport and explicit-save boundaries; preserve commit receipts and query uncertain transaction outcomes. Retain corrected R5/R6/R7 delivery and host enforcement. Scope/apply checks remain in domain operations, not duplicated in UI controls.
3. Rerun the affected unit controls and three-engine browser sequences, inspect decoded storage and final outbound requests, then resume remaining M4 cases and necessary build/version regressions. M5 records evidence and rollback readiness only; it is not permission to release.
...
- Overall: FAIL for implementation conformance, not a call for automatic rollback. Highest-value next action is the bounded M1 correction during the resumed implementation.
