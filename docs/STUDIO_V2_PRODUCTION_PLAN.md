...
| Current `npm run doctor` | **5/5 high-level checks passed**; **106 files / 567 tests**, production build and all three static pilot validators completed | Prior 535-test doctor and 536-test standalone runs are historical; this local result still does not close the 35 P0 cases or authorize release |
...
| Current Demo Gateway transport regression | **28/28 focused Demo unit tests**, **6/6 public-gateway browser controls** across Chromium/Firefox/WebKit, and **2/2 Chromium AI settings/locale controls** passed with synthetic intercepted requests | Confirms the no-key `/demo/session` → provider-tool-free `/demo/v1/responses` path, restricted media projection and final request body; live CORS, model aliases, Turnstile and Provider reliability remain unverified |
 | Latest PROD-03 readiness browser regression | **03-01 through 03-08 passed 3/3 each** across Chromium, Firefox and WebKit; the focused render/controller/CommandBus/context/status set passed **30/30**, the focused studio-file-export retry set passed **11/11**, UI i18n passed **7/7**, and the final serial unit run passed **106 files / 567 tests**. The site rebuild passed with the pinned PI-00 bundle at 823439 bytes and 202 service-worker precache entries | The 03-01..08 case matrix is closed; S10/PROD-08 is separately closed, while physical print/platform, live Provider and release approval remain open |
