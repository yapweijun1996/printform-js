...
| Current Demo Gateway transport regression | **28/28 focused Demo unit tests**, **6/6 public-gateway browser controls** across Chromium/Firefox/WebKit, and **2/2 Chromium AI settings/locale controls** passed with synthetic intercepted requests; the real Demo browser requalification passed session/models, design, layout review, seven media variants and privacy checks | Confirms the no-key `/demo/session` → provider-tool-free `/demo/v1/responses` path, restricted media projection and final request body; it is supporting-only and does not qualify direct-BYOK PI-04 reliability/retention |
