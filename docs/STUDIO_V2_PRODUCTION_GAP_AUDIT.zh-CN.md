...
## 0.2 Production Verification 证据快照
...
| Quality/layout/release coverage is incomplete | S11 closes actionable Quality page/component/field routing, keyboard and locale behavior; S12 closes retained current-workspace editing, desktop/mobile, locale, focus and rail acceptance; S13 closes bounded JS maintainability; CI now has three explicit pilot validations while the full browser/print matrix is incomplete | PROD-10/12; alternative workspace reorganization and independent Changes/history search remain open |
...
- **Pilot 样本门仍不同步**：`build-site` 与 `.github/workflows/ci.yml` 现在包含 Progress Claim 的静态 validator，但 `scripts/browser-matrix.mjs` 仍只覆盖 Sales Invoice/Purchase Order；Progress Claim 也尚未进入多表、签名、总额和页面边界矩阵。
...
| Pilots | Three built/static-validated pilots; Progress Claim Chromium coverage | CI now explicitly validates all three; full browser-matrix script still covers Invoice/PO |
...
1. S13-S16 retain local closure; S17/PI-04 remains the active dependency-ready acceptance step. X-01..03 isolated evidence is supporting only. S10-S12 retained local criteria remain closed; follow [handoff](STUDIO_V2_AGENT_HANDOFF.md) and TASK for current gate credit.
