...
- The current targeted matrix passes seven tests and dispatches all 35 registered commands through the safe envelope, including root-field checks, recursive unknown-field canaries, no-op/direct-approval variants, lifecycle/reference round-trips, null/malformed-input handling and safe malformed/stale errors. `tests/studio-v2/agent-output-boundary.test.js` additionally covers undo metadata, direct-root transactions, all history arrays, FormSpec/business-label redaction, neutral table labels, reviewedRevision provenance, malformed envelopes and wrong-type required arrays. Review media now carries local-only provenance into `buildProviderInput`; the final Provider parts are rebuilt from the allowlist and strip that provenance. `e2e/studio-v2-provider-wire.spec.js` captures the actual second Provider request after a safe Agent result in all three Playwright engines. This closes the catalog-dispatch implementation check and one composed wire path, but is not a case-specific P0 record or proof of every command/result root or destination/privacy combination.
