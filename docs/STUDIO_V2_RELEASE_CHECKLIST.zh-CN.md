...
>
> Last reviewed: 2026-09-10. The [direction review](STUDIO_V2_DIRECTION_REVIEW.md) keeps M1 implementation acceptance Partial. P0 remains 35 Pass, 0 Fail, and 0 Not run. The current `npm run doctor` passed 5/5 with 106 files / 571 tests (see [implementation evidence](STUDIO_V2_IMPLEMENTATION_EVIDENCE.md)); the current-source P0/PI-04 Playwright target run passed 36/36 and the local four-target browser matrix passed 88/88. The earlier 54/54 combined run included PI-03 and used the pre-a4caf93 runner/source baseline. The broader Demo Gateway design/review/media/privacy run is supporting baseline evidence from e430; current-source Demo evidence now includes focused 13-07 plus two real Chromium probes: session 201 without Authorization, two Demo Responses 200, Preview/UI Apply `r0` → `r1`, layout review pass, and production download fallback with revision-1 attestation validation PASS. No raw secret/body or exported HTML was retained; download completion, native Save As and system print preview remain unverified. Direct-BYOK PI-04 provider acceptance, platform/print checks, remote CI, deployment/retention and maintainer approval remain open. These results do not authorize release. Preserve private UI approval, host-bound admission and human production-export confirmation.
>
> 2026-09-11 addendum: current-source 13-07 revalidation passed 3/3 across Chromium, Firefox and WebKit; the AI/Trace UI regression passed 26/30 with four intentional browser-specific skips. This revalidates the credential-free Demo surface and registered-origin session path only; it does not change S17 G1/10%, direct-BYOK provider/platform status or release approval.
