...
export const TOOL_CONTRACTS = Object.freeze([
  ...
  { name: "set_locale", description: "Set the active print locale through a human-approved document mutation and invalidate prior review evidence.", inputSchema: objectSchema({ expectedRevision: revision, locale }, ["expectedRevision", "locale"]) },
  { name: "set_asset_source", description: "Replace one declarative image asset slot through a human-approved document mutation and revalidate it.", inputSchema: objectSchema({ expectedRevision: revision, slot: { type: "string", pattern: "^[a-z][a-z0-9-]*$" }, source: { type: "string", minLength: 1 } }, ["expectedRevision", "slot", "source"]) },
  { name: "set_sample_scenario", description: "Replace synthetic sample rows through a human-approved document mutation.", inputSchema: objectSchema({ expectedRevision: revision, scenario: { type: "string", enum: ["default", "empty", "one", "45-rows", "100-rows", "500-rows", "long-text"] } }, ["expectedRevision", "scenario"]) },
  { name: "undo_revision", description: "Select the prior logical draft state through a human-approved mutation and assign a new durable revision.", inputSchema: objectSchema({ expectedRevision: revision }, ["expectedRevision"]) },
