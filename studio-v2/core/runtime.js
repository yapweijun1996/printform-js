...
export function installPrintFormDocument(globalScope = globalThis) {
  ...
  async function render(data, options = {}) {
    ...
    const result = { status: combined.valid ? "ready" : "blocked", validation: combined, binding: bound.report, metrics: { ...layout.metrics, rows: bound.report.rows, durationMs: performance.now() - startedAt } };
