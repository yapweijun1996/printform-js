...
import {
  ...
  requireArray, requireObject, safeBoolean, safeCode, safeCount, safeHash, safeRevision, safeScenario, safeTime,
...
function scenarios(values, name = "scenarios") { return requireArray(values, name).map(safeScenario).filter(Boolean); }
...
function projectEvidenceReference(evidence, context) {
  ...
  return compact({
    evidenceId: ref(context, "evidence", evidence.evidenceId), scenario: safeScenario(evidence.scenario), candidateHash: safeHash(evidence.candidateHash, true), baseProjectHash: safeHash(evidence.baseProjectHash, true), layoutFingerprint: safeHash(evidence.layoutFingerprint, true), renderReportHash: safeHash(evidence.renderReportHash, true), snapshotHash: safeHash(evidence.snapshotHash, true), visualMode, pixelSnapshotHash: visualMode === "pixels" ? safeHash(evidence.pixelSnapshotHash, true) : undefined, coverage: projectCoverage(evidence.coverage, true), createdAt: safeTime(evidence.createdAt, true),
  });
}
...
function projectReviewReceipt(review, context) {
  ...
  return compact({ status: "pass", reviewedRevision: safeRevision(review.reviewedRevision, true), reviewer: "ai-agent", browsers: requireArray(review.browsers, "review browsers").map(projectBrowser).filter(Boolean), scenarios: scenarios(review.scenarios), evidence: requireArray(review.evidence, "review evidence").map((item) => projectEvidenceReference(item, context)), findings: requireArray(review.findings, "review findings").map((item) => projectReviewFinding(item, context)), attempt: safeCount(review.attempt, true), reviewedAt: safeTime(review.reviewedAt, true), metrics: projectMetrics(review.metrics) });
}
...
function projectLayoutCapture(result, context) {
  ...
  if (result.evidence) {
    ...
    output.requiredScenarios = scenarios(result.requiredScenarios, "required scenarios");
    output.capturedScenarios = scenarios(result.capturedScenarios, "captured scenarios");
    return output;
  }
...
export function projectAgentResult(name, result, context) {
  ...
  switch (name) {
    ...
    case "list_components": return { revision: safeRevision(result.revision, true), components: requireArray(result.components, "components").map((item) => projectComponent(item, context)) };
    ...
    case "list_active_transactions": return { transactions: requireArray(result.transactions, "active transactions").map((item) => projectTransaction(item, context)) };
    ...
    case "get_audit_events": return { events: requireArray(result.events, "audit events").map((item) => projectAudit(item, context, projectPack)) };
    ...
    case "get_transaction_history": return { revision: safeRevision(result.revision, true), entries: requireArray(result.entries, "journal entries").map((item) => projectJournal(item, context, projectPack)), transactions: requireArray(result.transactions, "history transactions").map((item) => projectTransaction(item, context)), auditEvents: requireArray(result.auditEvents, "history audit events").map((item) => projectAudit(item, context, projectPack)) };
    ...
    case "get_layout_review_status": return { revision: safeRevision(result.revision, true), review: projectReviewStatus(result.review, context), checklist: requireArray(result.checklist, "review checklist").filter((item) => typeof item === "string").map(() => "review-checklist-item") };
    case "begin_layout_review": return { revision: safeRevision(result.revision, true), attempt: safeCount(result.attempt, true), checklist: requireArray(result.checklist, "review checklist").filter((item) => typeof item === "string").map(() => "review-checklist-item"), requiredScenarios: scenarios(result.requiredScenarios, "required scenarios"), metrics: projectMetrics(result.metrics), issues: requireArray(result.issues, "review issues").map((item) => projectIssue(item, context)).filter(Boolean) };
...
export function sanitizeAgentResponse(name, response, options = {}) {
  const invalid = { ok: false, error: { code: "AGENT_OUTPUT_INVALID", message: "Agent output could not be projected safely" } };
  if (!response || typeof response !== "object" || Array.isArray(response) || ![true, false].includes(response.ok)) return invalid;
  try {
    if (response.ok === false) return { ok: false, error: projectAgentError(response.error, options.context || options.projectionContext) };
...
function projectAgentError(error = {}, context) {
  if (!error || typeof error !== "object" || Array.isArray(error)) error = {};
