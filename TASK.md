...
> Last reviewed: 2026-09-08, coding resumed. **Implementation Partial; R1-R3 host/session corrections and R5/R6 prompt corrections are implemented with case-specific evidence.** Actual Provider testing also found and corrected missing runtime prompt delivery (R7). See [resumed implementation evidence](docs/STUDIO_V2_IMPLEMENTATION_EVIDENCE.md) for current runs and remaining gates. P0 register is now 8 Pass (13-01/02/03/04/05/06/07/08), 0 Fail, and 27 Not run after the latest three-engine lifecycle and explicit-save runs. Runtime 1.0.0, Studio 0.11.0, Protocol 2.0.0, Agent Contract 4.0.0; 35 public commands. Production Ready is not established.
>
...
The [priority acceptance checklist](docs/STUDIO_V2_P0_ACCEPTANCE.md) owns the 35 cases: 8 Pass (13-01/02/03/04/05/06/07/08), 0 Fail, and 27 Not run. Production Ready is not established.
...
The [boundary/migration plan](docs/STUDIO_V2_AGENT_BOUNDARY_MIGRATION.md) retains the M0-M5 order. Coding resumed on 2026-09-08; bounded host/session, prompt, recipient and explicit-save corrections are now implemented and verified. Continue in this order: (1) complete remaining destination, commit-result and external-client boundaries; (2) close scope/apply/state and all mapped P0 cases; (3) investigate the long-run transaction timeout and prepare M5 evidence only after applicable gates. Preserve product defaults, public command scope, canonical state and human export approval; no deployment, push, data deletion or live Provider testing.

| ID | Priority / epic | Action and acceptance | Dependencies | Status |
|---|---|---|---|---|
| PROD-01 | P0 / E14 | Connect selection to FormSpec; reject out-of-scope edits in command path | Existing registry/gateway/operations | Partial: domain scope guard, stable table selection and project-replacement scope reset implemented; full selection matrix remains |
...
| PROD-08 | P0 / E14 | Preserve raw drafts; distinguish applied/recovery/saved/download-started; exercise failure paths | PROD-03; existing privacy policy | Partial: bounded explicit-save, denied-storage, uncertain-close and download controls pass; full overwrite/recovery/receipt acceptance remains |
...
- The prior complete Windows Playwright run passed 190/222 tests with 32 expected test-level skips and 0 failures; Chromium passed 74/74, Firefox 58/58 applicable, and WebKit 58/58 applicable. Current resumed bounded runs add 36/36 composed controls, 24/24 explicit-save controls and 6/6 recipient controls serially. A full four-target Windows release matrix, real printer chain and Safari.app certification are not available.
...
Continue M4 case-specific acceptance for PROD-13/01/02/03, especially the remaining scope/apply/state cases; complete external-client/WebMCP/CDP admission, overwrite/recovery/commit-result evidence and the server destination inventory, then investigate the long-run transaction timeout before preparing M5 rollback/release evidence. Do not deploy or publish without separate authorization.
