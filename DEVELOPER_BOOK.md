...
- `formatAllPrintForms()` in `src/printform.js` clones the `.printform`, resolves `data-*` config, and stitches the output page by page (core pagination lives in `src/printform/formatter/`).
...
- Public API stays in `src/printform.js`; helpers live in `src/printform/*.js`.
- The distributable single-file bundle is `dist/printform.js` (generated by `vite build` / `npm run build`).
- New flags belong in `CONFIG_DESCRIPTORS` (`src/printform/config.js`) plus the `.printform` comment block in the relevant HTML sample.
- Extend `collectSections()` (`src/printform/formatter/sections.js`) any time you invent a new section class or `data-*` flag.
