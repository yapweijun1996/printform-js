...
flowchart TD
    ...
    subgraph Processing [PrintForm.js 处理逻辑]
        Start(开始格式化) --> InitPage[创建新页面 Page 1]
        InitPage --> AddHeader[插入 Header & DocInfo & RowHeader]
        AddHeader --> LoopRows{还有剩余数据行?}
        
        LoopRows -- 是 --> CheckSpace{当前页还能塞下吗?}
        
        CheckSpace -- 能 --> AppendRow[插入数据行 .prowitem]
        ...
        LoopRows -- 否 (没数据了) --> FillDummyFinal[填充剩余空白行]
        FillDummyFinal --> AddFooterFinal[插入最终页脚]
        AddFooterFinal --> Finish(完成)
    end
