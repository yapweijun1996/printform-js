...
test.describe("Studio v2 PROD-01 01-04 mixed batch", () => {
  test("rejects allowed-plus-forbidden batches atomically", async ({ page }) => {
    const browserErrors = [];
    ...
    await frame.locator("table.scope-table-a-header").first().click();
    ...
    await page.evaluate((baselineRevision) => {
      const control = { runs: 0, actionCalls: 0, submitted: [], errors: [] };
      let runtimeOptions;
      const session = {
        ...
        runStream() {
          ...
          return (async function* () {
            ...
            control.submitted.push(operations);
            try {
              await action.execute({}, { expectedRevision: baselineRevision, operations });
            } catch (error) {
    ...
    const expectMixedRejected = async (operations, prompt) => {
      await page.evaluate((nextOperations) => { window.__p0Prod0104.control.nextOperations = nextOperations; }, operations);
    ...
    const allowedA = { type: "set_column_widths", tableSelector: ".scope-table-a-header, .scope-table-a-row", widths: ["12%", "43%", "11%", "16%", "18%"] };
    await expectMixedRejected([allowedA, { type: "set_column_widths", tableSelector: ".scope-table-b-header", widths: ["12%", "43%", "11%", "16%", "18%"] }], "Resize table A and table B together");
    await expectMixedRejected([allowedA, { type: "set_brand_color", hex: "#854d0e" }], "Resize table A and change the theme together");

    const control = await page.evaluate(() => window.__p0Prod0104.control);
    expect(control.runs).toBe(2);
    expect(control.actionCalls).toBe(2);
    expect(control.submitted).toHaveLength(2);
    expect(control.submitted.every((operations) => Array.isArray(operations))).toBeTruthy();
    expect(control.submitted[0]).toHaveLength(2);
    expect(control.submitted[1]).toHaveLength(2);
