...
test("presents a chat-first AI Designer with provider setup tucked away", async ({ page }) => {
  ...
  await expect(page.locator("#ai-model")).toHaveValue("demo-fast");
...
test("localizes provider settings modal across all supported languages", async ({ page, browserName }) => {
  ...
  const expected = {
    "en-MY": { title: "Provider settings", save: "Save changes", provider: "Provider", navLabel: "Provider setting sections", profileState: "Default gateway: demo-fast · browser demo session", status: "Demo Gateway ready · a short-lived origin-bound session is acquired on demand." },
    "zh-CN": { title: "提供商设置", save: "保存更改", provider: "提供商", navLabel: "提供商设置分区", profileState: "默认 Gateway：demo-fast · 浏览器 Demo 会话", status: "Demo Gateway 已就绪 · 按需获取短时、绑定来源的会话。" },
    "ms-MY": { title: "Tetapan pembekal", save: "Simpan perubahan", provider: "Pembekal", navLabel: "Bahagian tetapan pembekal", profileState: "Gateway lalai: demo-fast · sesi Demo pelayar", status: "Gateway Demo sedia · sesi ringkas terikat Origin diperoleh atas permintaan." },
    "ja-JP": { title: "プロバイダー設定", save: "変更を保存", provider: "プロバイダー", navLabel: "プロバイダー設定セクション", profileState: "既定ゲートウェイ：demo-fast · ブラウザーDemoセッション", status: "Demo Gatewayは準備完了です。短期でOriginに束縛されたセッションをオンデマンド取得します。" },
    "vi-VN": { title: "Cài đặt nhà cung cấp", save: "Lưu thay đổi", provider: "Nhà cung cấp", navLabel: "Các mục cài đặt nhà cung cấp", profileState: "Gateway mặc định: demo-fast · phiên Demo trên trình duyệt", status: "Gateway Demo đã sẵn sàng · phiên ngắn hạn gắn với Origin sẽ được lấy khi cần." }
  };
